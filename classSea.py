...
class Ship(object):
    ...
    def reroll(self, t):
        ...
        if self.direction is True:
            if (rein_x < (10-self.length)) and (rein_y < 10):
                self.y = rein_y
                self.x = rein_x
        else:
            if (rein_y < (10-self.length)) and (rein_x < 10):
                self.y = rein_y
                self.x = rein_x
...
class EnemySea(object):
    ...
    def __init__(self):
        self.grid_en = [["."] * 10 for _ in range(10)]
        # self.ships = []
        self.totalHp = 2 + 3 + 3 + 4 + 5
        self.lenghts = [2, 3, 3, 4, 5]
        self.flag = None
...
class YourSea(object):
    ...
    def __init__(self):
        self.grid_yo = [["."] * 10 for _ in range(10)]
        self.ships = []
        self.totalHp = 2 + 3 + 3 + 4 + 5
